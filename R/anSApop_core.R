anSApop_core <- function(SApopdat, title.ref, xlsx=FALSE, 
  ...
  cdlst <- na.omit(sort(ref_fortypgrpcd$VALUE))
  ...
  #######################################################################################
  tabnm <- "01"
  outfn.pre <- ifelse(is.null(outfn.pre), tabnm, paste0(tabnm, "_", outfn.pre))
  landarea <- "FOREST"
  estvar <- "TPA_UNADJ"
  ...
  estdat <- modSAtree(SApopdat=SApopdat, landarea=landarea, pcfilter=pcfilter,
		...
		allin1=allin1, title.ref=title.ref, title.dunitvar="fire", 
		outfolder=outfolder, outfn.pre=outfn.pre, outfn.date=outfn.date, 
		overwrite_layer=overwrite, title.filter=fortypgrpnm)
  #estdat$est
  esttab <- estdat$est
  tabtitle <- estdat$titlelst$title.estpse
