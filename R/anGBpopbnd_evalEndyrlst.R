anGBpopbnd_evalEndyrlst <- function(evalEndyrlst, data_dsn=NULL, 
		istree=TRUE, isseed=FALSE, ppsnm="pop_plot_stratum_assgn",
		pjoinid="CN", bnd_layer, bnd_dsn=NULL, bnd.att=NULL, 
		bnd.filter=NULL, xy_dsn=NULL, xy_layer="xyCur_ACTUAL", 
		xy.uniqueid="PLOT_ID", xvar="LON_ACTUAL", yvar="LAT_ACTUAL", 
		xy.crs=4269, strat_layer=NULL, strat_lut=NULL, savedata=FALSE, 
		outfolder=NULL, overwrite=FALSE) {
  ...
  ## Check evalidlst
  #########################################################################
  evalidall <- unlist(FIESTA::DBgetEvalid(evalid=unlist(evalidlst))$evalidlist)
  ...
  ## Add a temporary table to data_dsn
  #########################################################################
  data_conn <- DBI::dbConnect(RSQLite::SQLite(), data_dsn)
  DBI::dbWriteTable(data_conn, "ppsatmp", pltassgn, overwrite=TRUE)
  DBI::dbDisconnect(data_conn)
  ...
  for (evalyr in names(evalidlst)) {
    message("getting population data for ", toString(evalyr))

    ## Get population data for boundary
    #########################################################################
    GBpopdat <- modGBpop(cond="cond", plt="plot", tree="tree", seed="seed",
		pltassgn="ppsatmp", pltassgnid=pltassgnid, evalid=evalidlst[[evalyr]],
		...
		outfolder=NULL, outfn.pre=evalyr, outfn.date=FALSE, overwrite=overwrite)
 
    GBpop_evalEndyrlst[[evalyr]] <- GBpopdat
  }
  ...
  #########################################################################
  data_conn <- DBI::dbConnect(RSQLite::SQLite(), data_dsn)
  DBI::dbRemoveTable(data_conn, "ppsatmp")
  DBI::dbDisconnect(data_conn)
