...
#' @param grmtype String. The type of grm estimates ('GROW', 'MORT', 'REMV').
...
#' @param eststatus String. Status of species. ('Sawtimber', 'Growing-stock', 'All live').
...
modGBgrm <- function(GBpopdat, 
  ...
  formallst <- c(names(formals(modGBgrm)),
  ...
  grmx <- GBpopdat$grmx
  beginx <- GBpopdat$beginx
  midptx <- GBpopdat$midptx
  grow_typ_cd <- GBpopdat$grow_typ_cd
  mort_typ_cd <- GBpopdat$mort_typ_cd
  ...
  tdomdat <- treedat$tsumdat
  estvar.name <- treedat$sumvar
