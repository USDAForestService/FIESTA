modMAarea <- function(cond, pltmodel=NULL, cuniqueid="PLT_CN", puniqueid="CN", 
	...
	unitvar2=NULL, autocombine=FALSE, unitarea=NULL, areavar="ACRES", unitlut=NULL, 
	npixelvar=NULL, prednames=NULL, predfac=NULL, PSstrvar=NULL, rowvar=NULL, 
	rowvar.filter=NULL, colvar=NULL, colvar.filter=NULL, row.FIAname=FALSE,
 	col.FIAname=FALSE, row.orderby=NULL, col.orderby=NULL, row.add0=FALSE, col.add0=FALSE,
 	rowlut=NULL, collut=NULL, rowgrp=FALSE, rowgrpnm=NULL, rowgrpord=NULL, allin1=FALSE, 
	estround=0, pseround=3, estnull=0, psenull="--", divideby=NULL, savedata=FALSE,
  ...
  ###################################################################################
  datcheck <- check.data(gui=gui, esttype=esttype, module="MA", method=MAmethod, 
	cond=cond, plt=pltmodel, cuniqueid=cuniqueid, 
  ...
  if (adj == "samp") {
    ...
    adjfacdata <- FIESTA::getadjfactorGB(esttype=esttype, condx=condx, 
		cuniqueid=cuniqueid, condid=condid, unitlut=unitlut, unitvars=unitvars, 
		strvars=PSstrvar, unitarea=unitarea, areavar=areavar, 
    ...
    adjfacdata <- FIESTA::getadjfactorPLOT(esttype=esttype, condx=condx, 
		cuniqueid=cuniqueid)
    condx <- adjfacdata$condx
    unitlut <- adjfacdata$unitlut
    expcondtab <- adjfacdata$expcondtab
  ...
  ## GENERATE ESTIMATES
  #####################################################################################
  unit.totest=unit.rowest=unit.colest=unit.grpest=rowunit=totunit=domvar2=grpvar <- NULL
  ...
  if ("GREG" %in% MAmethod && !is.null(predfac)) {
    ...
    for (fac in predfac) {
      ...
      prednames <- c(prednames[prednames != fac], facvars)
    }
  }
  ...
  if (addtotal) {
    ...
    unit.totest <- do.call(rbind, lapply(estunits, MAest.unit, 
		dat=condall, cuniqueid=cuniqueid, pltmodelx=pltmodelx, puniqueid=puniqueid, 
  ...
  if (rowvar != "TOTAL")
	...
	dat=condall, cuniqueid=cuniqueid, pltmodelx=pltmodelx, 
