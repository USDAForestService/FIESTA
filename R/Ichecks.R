...
check.matchval <- function(tab1, tab2, var1, var2=NULL, tab1txt=NULL, tab2txt=NULL, 
	gui=FALSE, returnvals=FALSE, stopifnotnull=FALSE, subsetrows=FALSE) {
  ...
  ## returnvals - returns the missing values
  ## stopifnotnull - stop if the number of missing values is greater than 1
  ...
  if (subsetrows && stopifnotnull) stopifnotnull <- FALSE
  if (subsetrows && returnvals) returnvals <- FALSE
  ...
  if (length(var1) > 1) {
    ...
    missval <- tab2.vals[which(!tab1.vals %in% tab2.vals)]
    ...
    if (nbr.missval == length(tab1.vals)) {
      ...
      if (subsetrows) {
        ...
        if (nbr.missval < 20) {
          ...
          message(paste("there are", nbr.missval, "missing values in", tab1txt))
        }
      }
    }
  ...
  if (stopifnotnull) stop("")    

  if (subsetrows) return(tab1) 
  if (returnvals) return(missval)  
}  
