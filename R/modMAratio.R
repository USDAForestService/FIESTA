...
#'                      predfac = "tnt")
#'                      
#' # Use GREG estimators to estimate the Per-Acre cubic foot volume of live trees by
#' # stand size.   
#' mod <- modMAratio(MApopdat,
#'                   estvarn = "VOLCFNET",
      ...
      if (!is.null(tdomvar)) {
        if (!is.null(tdomvar2)) {
          ...
          if (any(grpvar %in% names(cdomdat))) {
            ...
            cdomdatsum <- cdomdat[, lapply(.SD, sum, na.rm=TRUE), 
                                  by=c(nitvar, cuniqueid, prednames), .SDcols=estvard.name]
            tdomdatsum <- merge(tdomdatsum, cdomdatsum, by=c(unitvar, cuniqueid))
          }
        }
      } else {
