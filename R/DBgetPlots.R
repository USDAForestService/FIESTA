...
DBgetPlots <- function (states = NULL, 
  ...
  pcheck.params(input.params, savedata_opts=savedata_opts, 
                eval_opts=eval_opts, xy_opts=xy_opts)
      ...
      if (getxy) {
        ...
        if (xymeasCur) {
	      ...
	      if (savedata) {
	        ...
	        if (issp && exportsp) {
	          outsp_fmt <- ifelse(out_fmt == "csv", "shp", out_fmt)
...
#            seedx <- DT_NAto0(seedx, seednavars)
    ...
    if ((iseval || savePOP) && !is.null(pltx)) {
      ...
      ppsavars <- toString(c("PLT_CN", "STRATUM_CN", "EVALID", 
                             "STATECD", "ESTN_UNIT", "STRATUMCD"))
      ppsaqry <- paste0("SELECT ", ppsavars, 
	                     "\nFROM ", ppsafromqry, 
					             "\nWHERE statecd =", stcd)
      if (iseval) {
        if (subsetPOP) {
