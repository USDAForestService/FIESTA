check.popdataVOL <- function(tabs, tabIDs, pltassgnx, pltassgnid, module,
  ...
  COND_STATUS_CD=CONDID=CONDPROP_UNADJ=SUBPPROP_UNADJ=MICRPROP_UNADJ=MACRPROP_UNADJ=
	STATECD=cndnmlst=PROP_BASIS=ACI.filter=V1=condsampcnt=
	NF_COND_STATUS_CD=TPA_UNADJ=methodlst=condqry=treeqry=cfromqry=tfromqry=
...
#				"from", cfromqry, whereqry)
  ...
  ###############################################################################
  ## Check cond and merge with pltx
  ###############################################################################
