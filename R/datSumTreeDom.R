datSumTreeDom <- function(tree=NULL, seed=NULL, cond=NULL, plt=NULL, plt_dsn=NULL,
  ...
  COND_STATUS_CD=COUNT=tadjfac=CONDPROP_UNADJ=V1=samenm=SUBP <- NULL
  checkNApvars <- {}
  checkNAcvars <- {}
  checkNAtvars <- {}
  ...
  if (is.null(treex) && is.null(seedx)) stop("must include tree and/or seed table")
  if (addseed && is.null(seedx)) stop("if addseed=TRUE, must include seed table")
  if (addseed && is.null(treex)) addseed <- FALSE
...
#    }
  ...
  if (!seedonly && !is.null(seedx)) {
    if (!tuniqueid %in% names(seedx)) stop(paste(tuniqueid, "not in seed"))
    setkeyv(seedx, key(treex))
  }
  ...
  } else if (seedonly && !tsumvar %in% c(tuniqueid, tpavars)) {
    stop("tsumvar can only be PLT_CN (count) for seeds only")
  ...
  if (tsumvar == uniqueid) tfun <- length
