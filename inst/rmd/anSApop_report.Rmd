...
#est1 <- SAareadat1$est
raw1 <- SAareadat1$raw
totest1 <- raw1$dunit_totest
titlelst1 <- SAareadat1$titlelst

est1 <- totest1[!is.na(totest1[[estnm]]), 
            c(domain, "NBRPLT.gt0", estnm, senm, "pse", "AREAUSED")]
...
est1tot <- est1
est1tot[[estnm]] <- (est1tot[[estnm]] * est1tot$AREAUSED)
est1tot[[senm]] <- (est1tot[[senm]] * est1tot$AREAUSED)

datBarplot(x=est1tot, xvar=domain, yvar=estnm, 
           errbars=TRUE, sevar=senm, toplabelvar="NBRPLT.gt0", ylab="Acres")
...
smallbnd1 <- merge(smallbnd, est1, by=domain)
...
SAareadat2 <- modSAest(SApopdat=SApopdat, SApackage="JoSAE", 
	...
	estvar="VOLCFNET", estvar.filter="STATUSCD == 1", 
 	smallbnd.att=NULL, savedata=FALSE, rawdata=TRUE,
	returntitle=TRUE, multest=FALSE, title.ref=title.ref)
#est2 <- SAareadat2$est
raw2 <- SAareadat2$raw
totest2 <- raw2$dunit_totest
titlelst2 <- SAareadat2$titlelst

est2 <- totest2[!is.na(totest2[[estnm]]), 
                c(domain, "NBRPLT.gt0", estnm, senm, "pse", "AREAUSED")]
...
est2tot <- est2
est2tot[[estnm]] <- (est2tot[[estnm]] * est2tot$AREAUSED)
est2tot[[senm]] <- (est2tot[[senm]] * est2tot$AREAUSED)

datBarplot(x=est2tot, xvar=domain, yvar=estnm, 
           errbars=TRUE, sevar=senm, toplabelvar="NBRPLT.gt0", 
           ylab="Acres", divideby="million")
...
## Merge estimate to smallbnd
smallbnd2 <- merge(smallbnd, est2, by=domain)
...
SAareadat3 <- modSAest(SApopdat=SApopdat, SApackage="JoSAE", 
	...
	returntitle=TRUE, multest=FALSE, title.ref=title.ref)
raw3 <- SAareadat3$raw
totest3 <- raw3$dunit_totest
titlelst3 <- SAareadat3$titlelst

est3 <- totest3[!is.na(totest3[[estnm]]), 
            c(domain, "NBRPLT.gt0", estnm, senm, "pse", "AREAUSED")]

knitr::kable(est3, "simple", align=c("lrrrrr"), digits=digits,
             format.args=list(big.mark = ","))
...
est3tot <- est3
est3tot[[estnm]] <- (est3tot[[estnm]] * est3tot$AREAUSED)
est3tot[[senm]] <- (est3tot[[senm]] * est3tot$AREAUSED)
...
smallbnd3 <- merge(smallbnd, est3, by=domain)
...
SAareadat4 <- modSAest(SApopdat=SApopdat, SApackage="JoSAE", 
	SAmethod="area", esttype="TREE", landarea="FOREST", 
	estvar="VOLBFNET", estvar.filter="STATUSCD == 1", 
 	smallbnd.att=NULL, savedata=FALSE, rawdata=TRUE,
	returntitle=TRUE, multest=FALSE, title.ref=title.ref)
raw4 <- SAareadat4$raw
totest4 <- raw4$dunit_totest
titlelst4 <- SAareadat4$titlelst

est4 <- totest4[!is.na(totest4[[estnm]]), 
            c(domain, "NBRPLT.gt0", estnm, senm, "pse", "AREAUSED")]
...
est4tot <- est4
est4tot[[estnm]] <- (est4tot[[estnm]] * est4tot$AREAUSED)
est4tot[[senm]] <- (est4tot[[senm]] * est4tot$AREAUSED)
...
## Merge estimate to smallbnd
smallbnd4 <- merge(smallbnd, est4, by=domain)
