...
### Remove any objects from environment before running script ###
rm(list=ls())
...
if (is.null(yearT1)) stop("T1 is NULL")
if (is.null(yearT2)) stop("T2 is NULL")
if (is.null(outfn.pre)) outfn.pre <- AOInm


## Import ICE QAQC data
#ice.QAQC <- FIESTA::pcheck.table(ice.QAQCfn)
if (is.null(ice.QAQCfn)) ice.QAQC <- NULL

if (!is.null(ice.QAQC)) {
  if (!dir.exists(ice.QAQCfn)) stop(ice.QAQCfn, " does not exist")
