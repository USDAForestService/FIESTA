...
# AOInm <- "ecoM332"
# load("E:/workspace/FIESTA/FIESTA_SA/testfolder4/M332/WWWpopdat.rda")
# pcfilter <- NULL
# title.ref <- NULL
# fortypgrpcd <- 200
# estnm="JFH"
...
if (!eval) {
  ...
  intro_text2 <-
    paste0("Estimates were generated using the Forest Inventory ESTimation and Analysis (FIESTA) R package (Frescino et al. 2023) and are based on area-level small area estimators from the R packages. Data for this report were extracted from the Forest Inventory and Analysis (FIA) database (Burrill et al. 2021).")
}
...
smallbnd <- {}
for (i in 1:length(WWWpopdat)) {
  smallbnd <- rbind(smallbnd, WWWpopdat[[i]]$smallbnd)
  smallbnd.unique <- WWWpopdat[[i]]$smallbnd.unique
}  
...
dunitarea <- {}
for (i in 1:length(WWWpopdat)) {
  if (!"AOI" %in% names(WWWpopdat[[i]]$dunitlut)) {
    WWWpopdat[[i]]$dunitarea$AOI <- 1
    WWWpopdat[[i]]$dunitlut$AOI <- 1
    WWWpopdat[[i]]$condx$AOI <- 1
  }
    
  dunitlut <- rbind(dunitlut, WWWpopdat[[i]]$dunitlut)
  dunitarea <- rbind(dunitarea, WWWpopdat[[i]]$dunitarea)
  areavar <- WWWpopdat[[i]]$areavar
}
...
SAareadat1 <- modSAarea(WWWpopdat = WWWpopdat, 
                        SApackage = SApackage, 
                        SAmethod = SAmethod, 
                        landarea = "FOREST", 
...
SAareadat2 <- modSAtree(WWWpopdat = WWWpopdat, 
                        SApackage = SApackage, 
                        SAmethod = SAmethod, 
                        landarea = "FOREST", 
                        estvar = "VOLCFNET", 
...
SAareadat3 <- modSAtree(WWWpopdat = WWWpopdat, 
                       SApackage = SApackage, 
                       SAmethod = SAmethod, 
                       landarea = "FOREST", 
                       estvar = "BA", 
...
SAareadat4 <- modSAtree(WWWpopdat = WWWpopdat, 
                       SApackage = SApackage, 
                       SAmethod = SAmethod, 
                       landarea = "FOREST", 
                       estvar = "VOLBFNET", 
