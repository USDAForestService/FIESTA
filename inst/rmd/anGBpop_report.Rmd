...
## Get states in GBpopdat or that popfilter
states <- GBpopdat$states
invyrs <- unique(unlist(GBpopdat$invyrs))

if (!is.null(popfilter) && "STATECD" %in% names(GBpopdat$pltcondx)) {
  pltcondf <- datFilter(GBpopdat$pltcondx, xfilter=popfilter)$xf
...
## Photo text
photo_text <- paste0("Photo from the ", AOInm, ", ", title.states, ".")
...
nplotspop <- sum(stratalut[, sum(n.total, na.rm=TRUE), by=unitvars][["V1"]])
nplotspopf <- totest$NBRPLT.gt0


intro_text1 <- paste0("The area of ", AOInm, " located in ", title.states, " is ", format(round(areapopf), big.mark=","), " acres with ", round(area.for/areapopf * 100), " percent of the area classified as forest land, ", round(area.nf/areapopf * 100), " percent classified as nonforest land, and ", round(area.water/areapopf * 100), " percent as water.") 


if (!EVALIDator_match) {
intro_text2 <-
paste0("Estimates are calculated using the Forest Inventory ESTimation and Analysis (FIESTA) R package (Frescino et al. 2015) and are based on FIA's standard 'Green Book' estimators (Bechtold and Patterson, 2005). Data for this report were extracted from the Forest Inventory and Analysis (FIA) database for years ", invyrs2, " (Woudenberg et al. 2010). The estimates are based on population data from a custom boundary, including a total of ", format(nplotspop, big.mark=","), " FIA plots. All percent sampling errors are reported at 68% confidence level.")
...
intro_text2 <-
paste0("Estimates are calculated using the Forest Inventory ESTimation and Analysis (FIESTA) R package (Frescino et al. 2015) and are based on FIA's standard 'Green Book' estimators (Bechtold and Patterson, 2005). Data for this report were extracted from the Forest Inventory and Analysis (FIA) database for years ", invyrs2, " (Woudenberg et al. 2010). ", addtext, addtext2, "All percent sampling errors are reported at 68% confidence level.")
...
estdat_EST1 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		pfilter=pfilter, cfilter=cfilter, estvarn="TPA_UNADJ", 
		sumunits=TRUE, estvarn.filter="STATUSCD == 1 & DIA >= 1.0", 
...
estdat_EST3 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		pfilter=pfilter, cfilter=cfilter, estvarn="BA", 
...
estdat_EST4_v1 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		pfilter=pfilter, cfilter=cfilter, estvarn="TPA_UNADJ", estseed="only", 
		rowvar="FORTYPGRPCD", row.FIAname=TRUE,
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		title.filter=AOInm)
...
estdat_EST5 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		pfilter=pfilter, cfilter=cfilter, estvarn="VOLCFNET", 
...
estdat_EST9_v1 <- modGBarea(GBpopdat=GBpopdat, landarea="FOREST", 
		pfilter=pfilter, cfilter=cfilter,	rowvar="STDAGENM",
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
...
estdat_EST10 <- modGBarea(GBpopdat=GBpopdat, landarea="FOREST", 
		pfilter=pfilter, cfilter=cfilter, rowvar="FORTYPGRPCD", row.FIAname=TRUE,
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
...
estdat_EST11_v1 <- modGBtree(GBpopdat=GBpopdat, landarea="FOREST",
    sumunits=TRUE, pfilter=pfilter, cfilter=cfilter, estseed="add",
		estvar="TPA_UNADJ", estvar.filter=paste("STATUSCD == 1", spcd.filter, sep=" & "), rowvar="DIACL",
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		divideby="million", title.filter=AOInm,
...
estdat_EST11_v2 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST",
    sumunits=TRUE, pfilter=pfilter, cfilter=cfilter, estseed="add",
		estvarn="TPA_UNADJ", estvarn.filter=paste("STATUSCD == 1", spcd.filter, sep=" & "), rowvar="DIACL", 
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		title.filter=AOInm, title.rowvar=paste("diameter class of", title.spcd))
...
estdat_EST14 <- modGBarea(GBpopdat=GBpopdat, landarea="FOREST", 
		pfilter=pfilter, cfilter=cfilter, rowvar="DSTRBCD1", 
