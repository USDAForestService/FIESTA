...
popfilter <- NULL
if (!is.null(pfilter) || !is.null(cfilter)) {
  if (!is.null(pfilter)) {
    if (!is.null(cfilter)) {
       popfilter <- paste(pfilter, cfilter, sep=" & ")
    }
    popfilter <- pfilter
  } else {
    popfilter <- cfilter
...
estdat_EST1 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		cfilter=cfilter, estvarn="TPA_UNADJ", sumunits=TRUE,
...
estdat_EST2 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		cfilter=cfilter, estvarn="TPA_UNADJ", 
...
estdat_EST4_v1 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		cfilter=cfilter, estvarn="TPA_UNADJ", estseed="only", 
		rowvar="FORTYPGRPCD", row.FIAname=TRUE,
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		title.filter=AOInm)
...
# EST4_v2 - Number of live aspen seedlings (< 1.0 in dia) per acre by forest type group barplot.
...
estdat_EST5 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", 
		cfilter=cfilter, estvarn="VOLCFNET", 
...
estdat_EST7 <- modGBtree(GBpopdat=GBpopdat, landarea="FOREST", 
		...
		rowvar="FORTYPGRPCD", row.FIAname=TRUE,
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		divideby="million", title.filter=AOInm)
...
estdat_EST9_v1 <- modGBarea(GBpopdat=GBpopdat, landarea="FOREST", 
		cfilter=cfilter,	rowvar="STDAGENM",
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		title.rowvar="stand age class",	title.filter=AOInm,
		divideby="thousand")
...
estdat_EST10 <- modGBarea(GBpopdat=GBpopdat, landarea="FOREST", 
		cfilter=cfilter, rowvar="FORTYPGRPCD", row.FIAname=TRUE,
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		title.filter=AOInm, divideby="thousand")
...
estdat_EST11_v1 <- modGBtree(GBpopdat=GBpopdat, landarea="FOREST",
    sumunits=TRUE, cfilter=cfilter, estseed="add",
...
estdat_EST11_v2 <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST",
    sumunits=TRUE, cfilter=cfilter, estseed="add",
		...
		rawdata=TRUE, returntitle=TRUE, estround=2, pseround=0,
		title.filter=AOInm, title.rowvar="diameter class of aspen")
...
estdat_EST14 <- modGBarea(GBpopdat=GBpopdat, landarea="FOREST", 
		cfilter=cfilter, rowvar="DSTRBCD1", 
		row.FIAname=TRUE,	rawdata=TRUE, returntitle=TRUE, 
		estround=2, pseround=0, title.filter=AOInm)
