...
1. [Data tables and unique identifiers of plot](#dtab)  
2. [Estimation data](#edata)   
3. [Nonsample filters](#nonsamp)
...
* **seed**      - Seedling data - *required for modGBtree() or modGBratio() seedling estimates*.
...
* **pjoinid**   - Join variable in plot to match pltassgnid. Does not need to be uniqueid.


<a name="edata"></a>
**Estimation data**

Define information for estimation data: ACI, adjust for nonsampled conditions; post-stratify.

* **evalid**    - FIA Evaluation identifier for subsetting plots for population (e.g., EVALID == 491801).
* **invyrs**    - Inventory year(s) (e.g., c(2000, 2001, 2002)).
* **ACI**       - FALSE. Defines whether to include All Condition Inventory (ACI) conditions and associated tree data in estimates. If FALSE, a filter, COND_STATUS_CD == 1, is generated and addition filters are applied for removing nonsampled conditions (see cond.nonsamp.filter).
* **adj**       - Adjustment factors are calculated to account for nonsampled (nonresponse) conditions ('samp', 'plot', 'none'). If adj='samp', calculations are based on summed proportions of sampled conditions divided by the total plots in the strata/estimation unit. These adjustments are only calculated for annual inventory plots (designcd=1). If adj='plot', calculations are based on 1 divided by the summed proportions of sampled conditions in plot.
...
Nonsample filters remove nonsample conditions from the dataset. Nonsample filters are identified internally, unless otherwise specified.

* **plt.nonsamp.filter** - If plt.nonsamp.filter=NULL, a predefined filter for nonsampled (nonresponse) plots (PLOT_STATUS_CD != 3 is used. If a different filter is needed, define the filter to remove plots that are nonsampled.
* **cond.nonsamp.filter** - If cond.nonsamp.filter=NULL, a predefined filter for nonsampled (nonresponse) conditions is used (COND_STATUS_CD != 5; or if ACI=TRUE: is.na(NF_COND_STATUS_CD) | NF_COND_STATUS_CD != 5). If a different filter is needed, define the filter to remove conditions that are nonsampled.
...
* **plt.filter**     - Plot-level filter.
* **cond.filter**    - Condition-level filter.
...
* **GBpopdat$strlut**      - Data frame of stratification information by estimation unit. See below for variable descriptions.
* **GBpopdat$strvar**      - Name of strata variable in strlut and condx 
...
WYstrlut    | WY strata look-up table with pixel counts (P1POINTCNT) by strata and estimation unit
...
######################################################################################
# Usage:
# modGBpop(cond, plt = NULL, tree = NULL, pltassgn = NULL, dsn = NULL,  
#   cuniqueid = "PLT_CN", condid = "CONDID", puniqueid = "CN", tuniqueid = "PLT_CN",   
#   pltassgnid = "CN", pjoinid = "CN", evalid = NULL, invyrs = NULL, 
...
#   getwt = TRUE, getwtvar = "P1POINTCNT", stratcombine = TRUE, savedata = FALSE,  
...
########################################################################################
GBpopdat <- modGBpop(cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", unitarea=WYunitarea, 
    unitvar="ESTN_UNIT", stratalut=WYstrlut)
...
GBpopdat$strlut       # Strata information and adjustment factors
...
######################################################################################
# Usage:
# modGBarea(cond = NULL, plt = NULL, pltassgn = NULL, dsn = NULL, cuniqueid = "PLT_CN", 
...
#   row.add0 = FALSE, col.add0 = FALSE, rowlut = NULL, collut = NULL, rowgrp = NULL,  
...
area1 <- modGBarea(cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", unitarea=WYunitarea, 
    unitvar="ESTN_UNIT", stratalut=WYstrlut, landarea="FOREST", sumunits=TRUE)
...
area2a <- modGBarea(cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
...
area3a <- modGBarea(cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN",   
  unitvar="ESTN_UNIT", unitarea=WYunitarea, strata=TRUE, stratalut=WYstrlut,   
...
area3b <- modGBarea(cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
...
area3c <- modGBarea(cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN",
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
...
area5b <- modGBarea(cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", unitvar=NULL, 
  unitarea=sum(WYunitarea$ACRES), stratalut=WYstrlut, landarea="FOREST", 
  rowvar="FORTYPCD", colvar="STDSZCD", row.FIAname=TRUE, col.FIAname=TRUE, 
...
######################################################################################
# Usage:
# modGBtree(tree = NULL, cond = NULL, plt = NULL, pltassgn = NULL, seed = NULL, 
#   dsn = NULL, tuniqueid = "PLT_CN", cuniqueid = "PLT_CN", condid = "CONDID", 
#   puniqueid = "CN", pltassgnid = "CN", pjoinid="CN", evalid=NULL, invyrs=NULL,   
#   ACI = FALSE, adj = "samp", strata = TRUE, plt.nonsamp.filter = NULL,  
#   cond.nonsamp.filter = NULL, unitvar = NULL, unitvar2 = NULL, unitarea = NULL,  
#   areavar = "ACRES", unitcombine = FALSE, stratalut = NULL, strvar = "STRATUMCD", 
#   getwt = TRUE, getwtvar = "P1POINTCNT", stratcombine = TRUE, landarea="FOREST", 
#   plt.filter = NULL, cond.filter = NULL, estvar = NULL, estvar.filter = NULL,  
#   estvar.name = NULL, rowvar = NULL, colvar = NULL, row.FIAname = FALSE,    
...
#   rowgrpord = NULL, sumunits = FALSE, allin1 = FALSE, estround = 0, pseround = 2,     
...
#   title.ref = NULL, title.rowvar = NULL, title.colvar = NULL, title.unitvar = NULL, x
#   title.ref = NULL, title.unitvar = NULL, title.estvar = NULL, title.filter = NULL)
# 
# Description:
...
tree3a <- modGBtree(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  strata=TRUE, unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST",
...
## Try adding GBpopdat
tree3aa <- modGBtree(GBpopdat=GBpopdat, estvar="VOLCFNET", estvar.filter="STATUSCD == 1",
  landarea="FOREST", sumunits=TRUE, rowvar="FORTYPCD", colvar="STDSZCD", 
...
## Rows and columns; combine estimation units (sumunits=TRUE; allin1=TRUE)
tree3b <- modGBtree(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
  estvar="VOLCFNET", estvar.filter="STATUSCD == 1", rowvar="FORTYPCD", colvar="OWNGRPCD",
  row.FIAname=TRUE, col.FIAname=TRUE, sumunits=TRUE, allin1=TRUE)
tree3b$est

## Rows and columns; combine estimation units (sumunits=TRUE; rawdata=TRUE; returntitle=TRUE)
tree3c <- modGBtree(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
  estvar="VOLCFNET", estvar.filter="STATUSCD == 1", rowvar="FORTYPCD",colvar="STDSZCD", 
...
tree5b <- modGBtree(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN",  
  unitvar=NULL, unitarea=sum(WYunitarea$ACRES), stratalut=WYstrlut, landarea="FOREST", 
  estvar="VOLCFNET", estvar.filter="STATUSCD == 1", rowvar="FORTYPCD", colvar="STDSZCD", 	
...
####################################################################################
# Usage:
# modGBratio(tree = NULL, cond = NULL, plt = NULL, pltassgn = NULL, seed = NULL, 
#   dsn = NULL, tuniqueid = "PLT_CN", cuniqueid = "PLT_CN", condid = "CONDID", 
#   puniqueid = "CN", pltassgnid = "CN", pjoinid="CN", evalid=NULL, invyrs=NULL, ACI = FALSE, 
#   adj = "samp", strata = TRUE, plt.nonsamp.filter = NULL, cond.nonsamp.filter = NULL, 
#   unitvar = NULL, unitvar2 = NULL, unitarea = NULL, areavar = "ACRES",   
#   unitcombine = FALSE, stratalut = NULL, strvar = "STRATUMCD", getwt = TRUE, 
#   strwtvar = "P1POINTCNT", stratcombine = TRUE, landarea="FOREST", plt.filter = NULL,  
...
#   sumunits = FALSE, allin1 = FALSE, estround = 3, pseround = 2, estnull = 0,   
...
## Try adding GBpopdat
rat3aa <- modGBratio(GBpopdat=GBpopdat, landarea="FOREST", estvarn="VOLCFNET", 	
  estvarn.filter="STATUSCD==1", rowvar="FORTYPCD", colvar="STDSZCD", row.FIAname=TRUE, 
  col.FIAname=TRUE, sumunits=TRUE, rawdata=TRUE, returntitle=TRUE)
rat3aa$est
rat3aa$pse


## Rows only - Species per acre; combine estimation units (sumunits=TRUE)
rat4 <- modGBratio(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
  estvarn="VOLCFNET", estvarn.filter="STATUSCD==1", rowvar="FORTYPCD", row.FIAname=TRUE, 
  sumunits=TRUE)
rat4$est
...
rat5 <- modGBratio(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
...
rat6 <- modGBratio(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea,stratalut=WYstrlut, landarea="FOREST", 
...
## Tree ratio
## Rows only; combine estimation units (sumunits=TRUE; allin1=TRUE)
rat7 <- modGBratio(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, landarea="FOREST", 
  estvarn="VOLCFNET", estvarn.filter="STATUSCD == 1", estvard="VOLCFNET", rowvar="FORTYPCD", 
  row.FIAname=TRUE, sumunits=TRUE, allin1=TRUE)
rat7$est


rat7b <- modGBratio(tree=WYtree, cond=WYcond, pltassgn=WYpltassgn, pltassgnid="CN", 
  ratiotype="PERTREE", unitvar="ESTN_UNIT", unitarea=WYunitarea, stratalut=WYstrlut, 
  landarea="FOREST", estvarn="VOLCFNET", estvarn.filter="STATUSCD == 1", estvard="VOLCFNET", 
 rowvar="FORTYPCD", row.FIAname=TRUE,  sumunits=TRUE, allin1=TRUE)
rat7b$est
