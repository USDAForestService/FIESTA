#' Green-Book module - Generate land use/land cover estimates.
#' 
#' Generates estimates of land use/land cover by domain (and estimation unit).
#' Calculations are based on Scott et al. 2005 ('the green-book') for mapped
#' forest inventory plots. The non-ratio estimator for estimating area by
#' stratum and domain is used. Plots that are totally nonsampled are excluded
#' from estimation dataset. Next, an adjustment factor is calculated by strata
#' to adjust for nonsampled (nonresponse) conditions that have proportion less
#' than 1.  The attribute is the proportion of the plot which is divided by the
#' adjustment factor, and averaged by stratum. Strata means are combined using
#' the strata weights and then expanded to area using the total land area in
#' the population.
#' 
