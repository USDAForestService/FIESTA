...
dat7 <- DBgetPlots(states = "Vermont", 
                   eval = "FIA",
                   eval_opts = eval_options(Cur = TRUE, 
...
dat9 <- DBgetPlots(states = c("Texas", "Delaware", "Oregon"), 
                   eval = "FIA",
                   eval_opts = eval_options(Cur = TRUE, 
                                            evalType = "ALL",
                                            Endyr = 2017),
...
#### Example 13: Get data for most current FIA Evaluation and most current custom Evaluation
<details>
  <summary>View Example</summary>
...
UTevalCur <- DBgetPlots(states = "Idaho", 
                        eval = "FIA",
                        eval_opts = eval_options(Cur = TRUE,
                                                 evalType = "ALL"),
                        issp = TRUE
...
spxy.UTevalCur <- UTevalCur$xy_public
...
UTmeasCur <- DBgetPlots(states = "Idaho", 
                        eval = "custom",
                        eval_opts = eval_options(Cur = TRUE,
                                                 evalType = "ALL"),
                        issp = TRUE
...
spxy.UTmeasCur <- UTmeasCur$xy_public
...
dat14 <- DBgetPlots(states = "Vermont", 
                    eval = "FIA",
                    eval_opts = list(Cur = TRUE, 
                                     Type = "ALL"),
                    issp = TRUE
...
tabs14 <- dat14$tabs
plt14 <- tabs14$plt
spxy14 <- dat14$xy_public

## With only P2 plots (intensity1 = TRUE)
dat14b <- DBgetPlots(states = "Vermont", 
                     eval = "FIA",
                     eval_opts = list(Cur = TRUE, 
                                     Type = "ALL"),
                     intensity1 = TRUE,
...
table(plt14$INVYR)
table(plt14b$INVYR)
spxy14b <- dat14b$xy_public


# Display spatial output of public coordinates
plot(sf::st_geometry(spxy14), pch = 16, cex = .5)
plot(sf::st_geometry(spxy14b), pch = 16, cex = .5, add = TRUE, col = "grey")
...
#### Example 15: Filter multiple states for Region 1 forests
<details>
  <summary>View Example</summary>
...
dat15 <- DBgetPlots(states = "Montana", 
                    eval = "custom",
                    eval_opts = list(Cur = TRUE, 
                                     Type = "ALL"),
...
### Look at plot counts
dat15$pltcnt
table(dat15$tabs$cond$ADFORCD)

## Display points with by PLOT_STATUS_CD (1-light blue; 2-brown; 3-blue)
spxy15 <- dat15$xy_public

spxy15$color <- with(spxy15, 
          ifelse(PLOT_STATUS_CD == 2, "brown", 
                ifelse(PLOT_STATUS_CD == 3, "blue", "light blue")))
plot(sf::st_geometry(spxy15['PLOT_STATUS_CD']), pch = 16, cex = .5,
                     col = spxy15$color)


## Get plots that have pinyon pine trees in Montana
dat15b <- DBgetPlots(states = "Montana", 
                     eval = "custom",
                     eval_opts = list(Cur = TRUE, 
                                      Type = "ALL"),
                     allFilter = "OWNGRPCD == 10 & (ADFORCD > 100 & ADFORCD < 200)",
...
## Display points with by PLOT_STATUS_CD (1-light blue; 2-brown; 3-blue)
spxy15b <- dat15b$xy_public
plot(sf::st_geometry(spxy15b), pch = 16, cex = .5, col = "wheat", add = TRUE)
