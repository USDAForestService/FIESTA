...
A data table of individual or aggregated point data is required input to the modPB function. Plot-level information, such as: state, county, or strata assignment, may be included in pltstrat or in pnt, or pltpct with pltstrat=NULL. The parameter input can be the name of a comma-delimited file (\*.csv) or an R data frame or data table object already loaded into R. The pltstrat table can also be a point shapefile (\*.shp) or a SpatialDataFrame object with one point per plot. The unique identifier for the plots must be provided in pltstrat and pnt and the unique identifier for the points must additionally in pnt. See the required variables section for a list of variables necessary to include for estimation.
...
* **plotid** - Unique identifier for a plot in pnt or pntcnt. Values match puniqueid values in pltstrat.
...
* **pltstrat**  - Plot-level data, with 1 record per plot, including estimation unit and/or strata information. Optional.
* **puniqueid** - Unique identifier for a plot in pltstrat, if pltstrat is not NULL. 
...
icepltstrat_utco1135.csv | ICE plot-level data, including estimation unit and strata variables
...
icepltfn <- system.file("extdata", "PB_data/icepltstrat_utco1135.csv", package="FIESTA")
...
icepltsp <- spMakeSpatialPoints(iceplt, uniqueid="plot_id", x="LON_PUBLIC", y="LAT_PUBLIC", 
              prj="longlat", datum="NAD83")
sp::plot(icepltsp, col=factor(icepltsp$ESTN_UNIT))
...
#   pltstrat = NULL, puniqueid = "CN", ratio = FALSE, tabtype = "PCT", sumunits = FALSE, 
#   strata = FALSE, landarea = "ALL", landarea.filter = NULL, nonsamp.filter = NULL,
#   pnt.filter = NULL, plt.filter = NULL, unitarea = NULL, unitvar = NULL,   
#   areavar = "ACRES", stratalut = NULL, strvar = "STRATA", getwt = TRUE, 
#   getwtvar = "P1POINTCNT", autocombine = TRUE, rowvar = NULL, colvar = NULL, 
...
#   addtitle = TRUE, savetitle = FALSE, title.main = NULL, title.ref = NULL, 
...
cover1 <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)")
names(cover1)
cover1$est

# Note: pltstrat is not necessary for this estimate
...
chg_ag <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
...
chg_ag.area <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
chg_ag.plts <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
chg_ag.pnts <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
# Note: Add a landarea filter to subset dataset to only plots with observed change.
landarea.filter <- "change_1_2 == 1"
chg_ag.plts.area <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
# Note: Add a point filter to include observed change on points
pnt.filter <- "chg_ag_2 > 0"
chg_ag.pnts.area <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
		pnt.filter=pnt.filter, returntitle=TRUE, title.filter="observed changed", 
...
cover1.unit.area <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", 
...
cover1.unitsum <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
...
# Return raw data
# Note: Estimates are generated by ESTN_UNIT and summed to the population.
cover1.unitsum.area <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.filter="cover_1 != 999", tabtype="AREA", unitarea=unitarea, 
...
cover12 <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
...
cover12.acres <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, title.rowvar="Land Cover (2011)", title.colvar="Land Cover (2012)", 
		nonsamp.filter="cover_1 != 999", tabtype="AREA", unitarea=sum(unitarea$ACRES))
names(cover12.acres)
...
cover12nm <- modPB(pnt=icepnt2, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
...
cover12.lt200 <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, title.rowvar="Land Cover (2011)", title.colvar="Land Cover (2012)", 
...
cover12b <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
...
cover12ps <- modPB(pnt=icepnt, pltstrat=iceplt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
