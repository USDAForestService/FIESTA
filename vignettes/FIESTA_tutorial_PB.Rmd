...
A data table of individual or aggregated point data is required input to the modPB function. Plot-level information, such as: state, county, or strata assignment, may be included in pltassgn or in pnt, or pltpct with pltassgn=NULL. The parameter input can be the name of a comma-delimited file (\*.csv) or an R data frame or data table object already loaded into R. The pltassgn table can also be a point shapefile (\*.shp) or an sf spatial object with one point per plot. The unique identifier for the plots must be provided in pltassgn and pnt and the unique identifier for the points must additionally be in pnt. See the required variables section for a list of variables necessary to include for estimation.
...
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\Icheck.popdataPB.R")
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\modPBpop.R")
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\modPB.R")
...
```
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\Icheck.estdataPB.R")
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\Icheck.popdataPB.R")
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\modPBpop.R")
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\modPB.R")
...
chg_ag <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
...
chg_ag.area <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
chg_ag.plts <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
chg_ag.pnts <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
# Note: Add a landarea filter to subset dataset to only plots with observed change.
landarea.filter <- "change_1_2 == 1"
chg_ag.plts.area <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
# Note: Add a point filter to include observed change on points
pntfilter <- "chg_ag_2 > 0"
chg_ag.pnts.area <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
		pntfilter=pntfilter, returntitle=TRUE, title.filter="observed changed", 
...
chg_ag_cover1 <- modPB(pntdat=icepnt, pntid="dot_cnt", rowvar="chg_ag_2", 
...
chg_ag_cover1.acres <- modPB(pntdat=icepnt, pntid="dot_cnt", rowvar="chg_ag_2", 
...
chg_ag_cover2 <- modPB(pntdat=icepnt, pntid="dot_cnt", rowvar="chg_ag_2", 
    rowlut=icechg_ag, colvar="cover_2", collut=icecover_2, rawdata=TRUE, 
...
cover1.unitsum2 <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
...
# Return raw data
# Note: Estimates are generated by ESTN_UNIT and summed to the population.
cover1.unitsum.area <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", unitarea=unitarea, 
		unitvar="ESTN_UNIT", areavar="ACRES", sumunits=TRUE, rawdata=TRUE)
...
cover12 <- modPB(PBpopdat=PBpopdat,
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, title.rowvar="Land Cover (2011)", title.colvar="Land Cover (2012)", 
		nonsamp.pntfilter="cover_1 != 999", returntitle=TRUE)
names(cover12)
...
cover12.acres <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, title.rowvar="Land Cover (2011)", title.colvar="Land Cover (2012)", 
		nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", unitarea=sum(unitarea$ACRES))
names(cover12.acres)
