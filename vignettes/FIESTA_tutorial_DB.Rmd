...
#### Example 7: Get data for multiple states by Endyr
<details>
  <summary>View Example</summary>

```{r}
dat7 <- DBgetPlots(states = c("Connecticut", "South Carolina"), 
                   eval = "FIA",
                   eval_opts = eval_options(Cur = TRUE, 
                                            evalType = "ALL",
                                            Endyr = 2017))
...
dat10b <- DBgetPlots(states = "Vermont", 
                     eval = "FIA",
                     eval_opts = list(Cur = TRUE, 
                                     Type = "ALL"),
                     intensity1 = TRUE,
...
table(plt10$INVYR)
table(plt10b$INVYR)
spxy10b <- dat10b$xyCur_PUBLIC


# Display spatial output of public coordinates
plot(sf::st_geometry(spxy10), pch = 16, cex = .5)
plot(sf::st_geometry(spxy10b), pch = 16, cex = .5, add = TRUE, col = "grey")
