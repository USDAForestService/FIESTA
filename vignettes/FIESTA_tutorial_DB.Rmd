...
## FIESTA Overview
The R package, FIESTA (Forest Inventory ESTimation and Analysis) is a research estimation tool for analysts that work with sample-based inventory data such as that from the U.S. Department of Agriculture, Forest Service, Forest Inventory and Analysis (FIA) Program. FIESTA was designed to accommodate: unique population boundaries, different evaluation time periods, customized stratification schemes, non-standard variance equations, integration of multi-scale remotely-sensed data and other auxiliary information, and interaction with other modeling and estimation tools from CRAN R's library of packages. FIESTA contains a collection of functions that can access FIA databases, summarize and compile plot and spatial data, and generate estimates with associated sampling errors. 
...
The objective of this tutorial is to demonstrate the use of FIESTA's DB tools for accessing FIA data. These tools extract data from FIA Datamart using FIA's standard evaluations as well as customized evaluations (DBgetPlots and DBgetStrata). 
...
Note: FIESTA database tools can extract information for all FIA Evaluation Types, but FIESTA estimates are currently only available for area and tree. 
...
The following examples show how to extract data from FIA's publicly-available, online DataMart. Data can be returned as R objects or exported to CSV (.csv) files or a SQLite (.sqlite) database.  The zip files are extracted on-the-fly from the online website. Web server connections will affect download speeds.
...
# 
# Description:
# Queries and extracts data from FIA's online data sources
...
######################################################################################
# Usage:
# DBgetEvalid(states = NULL, RS = NULL, invtype = "ANNUAL", evalCur = TRUE,   
#   evalEndyr = NULL, evalid = NULL, evalAll = FALSE, evalType = "AREAVOL",
#   isveg = FALSE, isdwm = FALSE, invyrtab = NULL, gui = FALSE)
# 
# Description:
...
#   $SURVEY      - If returnPOP=TRUE, the SURVEY table from FIADB
#   $POP_EVAL - The popul
######################################################################################
...
NYTXeval.cur <- DBgetEvalid(states=c("New York", "Texas"), evalType=c("VOL", "GRM"))
names(NYTXeval.cur)
...
#   alltFilter = NULL, savedata = FALSE, outfolder = NULL, out_fmt = "csv", 
#   out_dsn = NULL, append_layer = TRUE, outfn.pre = NULL, outfn.date = FALSE,  
...
# Include a spatial object (sf class) with public coordinates
...
WYtabs.evalCur00 <- WYdat.evalCur00$tabs
WYplt.evalCur00 <- WYtabs.evalCur00$plt
WYcond.evalCur00 <- WYtabs.evalCur00$cond
...
##############################################
# Now, include tree data 
source("C:\\_tsf\\_GitHub\\FIESTA\\R\\DBgetPlots.R")
WYdat.evalCur00 <- DBgetPlots(states="Wyoming", evalCur=TRUE, issp=TRUE, istree=FALSE)
...
WYplt.evalCur00 <- WYtabs.evalCur00$plt
WYcond.evalCur00 <- WYtabs.evalCur00$cond
WYspxy.evalCur00 <- WYdat.evalCur00$spxy_PUBLIC
WYdat.evalCur00$evalid
WYdat.evalCur00$pltcnt
...
plot(sf::st_geometry(WYspxy.evalCur00))



## Add a plot/condition filter
##############################################
...
WYspxy.evalCur.wpftp <- WYdat.evalCur.wpftp$spxy_PUBLIC
...
# Look at spatial output
plot(st_geometry(WYspxy.evalCur00))
plot(st_geometry(WYspxy.evalCur.wpftp), add=TRUE, col="green")
...
WYdat.evalCur.wpsp <- DBgetPlots(states="Wyoming", evalCur=TRUE, issp=TRUE, 
    alltFilter="SPCD == 101")
names(WYdat.evalCur.wpsp)

## Extract and look at components in returned list object
WYspxy.evalCur.wpsp <- WYdat.evalCur.wpsp$spxy_PUBLIC
WYdat.evalCur.wpsp$pltcnt
dim(WYspxy.evalCur.wpsp)

# Look at spatial output
plot(st_geometry(WYspxy.evalCur00))
plot(st_geometry(WYspxy.evalCur.wpsp), add=TRUE, col="red")
plot(st_geometry(WYspxy.evalCur.wpftp), add=TRUE, col="green")
...
WYdat <- DBgetPlots(states="Wyoming", evalCur=TRUE, istree=TRUE, issp=TRUE,
              ...
              outfolder=outfolder, out_fmt="csv", overwrite_layer=TRUE)
...
WYdat <- DBgetPlots(states="Wyoming", evalCur=TRUE, issp=FALSE, istree=FALSE,
              othertables=c("SUBPLOT", "PLOTGEOM"), savePOP=TRUE, savedata=TRUE, 
              outfolder=outfolder, out_fmt="sqlite", out_dsn="data.db", 
...
WYdat <- DBgetPlots(states="Wyoming", evalCur=TRUE, issp=TRUE, istree=FALSE,
              othertables="PLOTGEOM", savePOP=TRUE, savedata=TRUE, 
              outfolder=outfolder, out_fmt="sqlite", out_dsn="datasp.db", 
              overwrite_layer=TRUE, overwrite_dsn=TRUE)
...
# Get data for RMRS and SRS, most current evaluation (Note: takes a long time if istree=TRUE)
##################################################################################
...
RMRS_SRSdat <- DBgetPlots(RS=c("SRS", "RMRS"), evalCur=TRUE, istree=FALSE)
names(RMRS_SRSdat)
...
# Most current evaluation for multiple evalTypes ('ALL', 'VOL', 'GRM') 
##################################################################################
WYdat1 <- DBgetPlots(states="Wyoming", evalCur=TRUE, evalType=c("ALL", "VOL", "GRM"),
                istree=TRUE)
names(WYdat1)
...
## PLT_CN values for Evaluation 561901 (evalType - VOL)
...
##################################################################################
dat <- DBgetPlots(evalid=c(561801, 491801, 81801))
names(dat)
...
table(dat2$plt$STATECD, dat2$plt$INVYR)
...
UTWYnfs.spxy$color <- with(UTWYnfs.spxy, 
          ifelse(PLOT_STATUS_CD == 2, "blue", 
                ifelse(PLOT_STATUS_CD == 3, "brown", "light blue")))
...
plot(sf::st_geometry(R4dat.t101.spxy), pch=16, cex=.5, col="wheat", add=TRUE)
...
#   $stratalut    - strata look-up table with summarized pixel counts (P1POINTCNT)
