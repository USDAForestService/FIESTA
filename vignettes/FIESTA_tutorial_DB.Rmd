...
## Get most current evaluation for Oregon and Texas for areavol and grm evalTypes
ORTXeval.cur <- DBgetEvalid(states=c("Oregon", "Texas"), evalType=c("AREAVOL", "GRM"))
names(ORTXeval.cur)
ORTXeval.cur$evalidlist
ORTXeval.cur$evalTypelist
...
WYdat1a <- DBgetPlots(datsource="CSV", states="Wyoming", evalCur=TRUE,
                    issp=TRUE, evalType=c("ALL", "AREAVOL", "GRM"))
names(WYdat1a)
WYplt1a <- WYdat1a$plt
...
##################################################################################
UTWYdata.nfs <- DBgetPlots(datsource="CSV", states=c("Utah", "Wyoming"), 
    evalEndyr=2017, allFilter="OWNCD == 11", evalType="ALL")
names(UTWYdata.nfs)
UTWYdata.nfs$pltcnt
...
# Include tree data
##################################################################################
UTdata.nfs <- DBgetPlots(datsource="CSV", states=c("Idaho", "Montana"), 
    invyrs=2008:2017, istree=TRUE, 
...
##################################################################################
UTdata.nfsACI <- DBgetPlots(datsource="CSV", states=c("Idaho", "Montana"), 
    invyrs=2008:2017, istree=TRUE, ACI=TRUE,
    allFilter="OWNGRPCD == 10 & (ADFORCD > 100 & ADFORCD < 200)")
names(UTdata.nfsACI)
