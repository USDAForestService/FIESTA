...
#   out_fmt = "csv", out_dsn = NULL, outfn.pre = NULL, outfn.date = TRUE, overwrite_dsn = FALSE,  
#   overwrite_layer = TRUE, GBdata = NULL, pltdat = NULL, GBstratdat = NULL, gui = FALSE)
# 
# Description:
...
area.bhdist <- modGBarea(GBpopdat=GBpopdat.bhdist, 
    ...
    rawdata=TRUE, returntitle=TRUE, title.ref="Bighorn National Forest", savedata=TRUE, outfn.pre="02")
...
vol.bh <- modGBtree(GBpopdat=GBpopdat.bh, tree=WYtree, cond=WYcond, 
  rowvar="FORTYPCD", row.FIAname=TRUE, colvar="STDSZCD", col.FIAname=TRUE,
...
## List of raw data
vol.bh.raw <- vol.bh$raw
names(vol.bh.raw)

## Raw data for estimation unit
vol.bh$raw$unit.totest

## Raw data for row estimates
vol.bh$raw$unit.rowest

## Raw data for column estimates
vol.bh$raw$unit.colest

## Raw data for group (cell) estimates
head(vol.bh$raw$unit.grpest)
