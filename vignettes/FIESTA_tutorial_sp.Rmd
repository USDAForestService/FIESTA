...
WYspplt <- FIESTA::spMakeSpatialPoints(xyplt = WYplt,
                                       xy.uniqueid = "CN",
                                       xvar = "LON_PUBLIC", 
                                       yvar = "LAT_PUBLIC",
                                       prj = "longlat",
...
WYspplt.utm12 <- FIESTA::spReprojectVector(layer = WYspplt,
                                           crs.new = prj)
...
WYbhptslst <- FIESTA::spClipPoint(xyplt = WYplt, 
                                  uniqueid = "CN",
                                  clippolyv = WYbh, 
                                  xvar = "LON_PUBLIC", 
                                  yvar = "LAT_PUBLIC", 
...
```{r}
WYbhptslst <- FIESTA::spClipPoint(xyplt = WYspplt,
                                  uniqueid = "CN",
                                  clippolyv = WYbh, 
...
FIESTA::spExportSpatial(WYbhspplt,
                        out_layer = "WYbhpts", 
                        outfolder = outfolder, 
                        overwrite_layer = TRUE)
```
...
We first subsect the USAco `SpatialPolygons` layer with `WYbighorn`. Note: you must download USAco from the `raster` package. See the "Set Up" section above.
  
```{r}
WYbhco <- FIESTA::spClipPoly(polyv = USAco,
                             clippolyv = WYbh)
```
...
```{r}
WYbhMW.fornf <- FIESTA::spClipRast(fornffn,
                                   clippolyv = WYbhMW,
                                   outfolder = outfolder)
```
...
```{r}
WYbhMWprj <- FIESTA:::crsCompare(WYbhMW,
                                 FIESTA:::rasterInfo(WYbhMW.fornf)$crs)$x
...
```{r}
extpolylst <- FIESTA::spExtractPoly(WYspplt,
                                    uniqueid = "CN",
                                    polyvlst = WYbhdist)
...
```{r}
extrastlst <- FIESTA::spExtractRast(WYspplt,
...
The `spGetPlots` function extracts data from FIA database for plots within a given boundary.
...
modeldat <- FIESTA::spGetAuxiliary(xyplt = WYspplt,
                                   uniqueid = "CN",
                                   dunit_layer = WYbhfn,
                                   dunitvar = NULL,
...
```{r}
unitdat.bh <- FIESTA::spGetEstUnit(xyplt = WYplt, 
                                   uniqueid = "CN", 
                                   xvar = "LON_PUBLIC",
                                   yvar = "LAT_PUBLIC",
                                   xy.crs = 4269,
...
```{r}
unitdat.bhdist <- FIESTA::spGetEstUnit(xyplt = WYplt,
                                       uniqueid = "CN",
                                       xvar = "LON_PUBLIC",
                                       yvar = "LAT_PUBLIC", 
                                       xy.crs = 4269, 
...
```{r}
stratlst <- FIESTA::spGetStrata(WYspplt,
                                uniqueid = "CN", 
                                unittype = "POLY",
                                unit_layer = WYbhfn,
                                strattype = "RASTER", 
...
Note: you can also use `spGetAuxiliary` function for getting strata info (only if strata layer is raster).

```{r}
stratlst2 <- FIESTA::spGetAuxiliary(WYspplt,
                                    uniqueid = "CN",
	                                  dunit_layer = WYbhfn, 
                                    dunitvar = NULL,
	                                  rastlst.cat = fornffn,
                                    rastlst.cat.name = "STRATUMCD")
stratlst2$stratalut	
...
polyUnion <- FIESTA::spUnionPoly(polyv1 = USAco[USAco$NAME_1 == "Wyoming",], 
                                 polyv2 = WYbh, 
                                 areacalc=TRUE)
...
zonallst <- FIESTA::spZonalRast(polyv = WYbhdist,
