...
FIESTA::spExportSpatial(WYbh, out_dsn="WYbh.shp", 
                        outfolder=outfolder, overwrite_dsn=TRUE)
...
WYspplt <- spMakeSpatialPoints(xyplt=WYplt, xy.uniqueid="CN", xvar="LON_PUBLIC", 
        ...
        outfolder=outfolder, out_layer="WYplots", overwrite_layer=TRUE)
...
###################################################################################
# Usage:
# spClipPoint(xyplt, xyplt_dsn = NULL, uniqueid = "PLT_CN", 
...
WYbhptslst <- FIESTA::spClipPoint(xyplt=WYplt, uniqueid="CN", clippolyv=WYbh, 
...
WYspplt <- FIESTA::spMakeSpatialPoints(xyplt=WYplt, xy.uniqueid="CN", xvar="LON_PUBLIC", 
        yvar="LAT_PUBLIC", xy.crs=4269)
WYbhptslst <- spClipPoint(xyplt=WYspplt, uniqueid="CN", clippolyv=WYbh)
...
FIESTA::spExportSpatial(WYbhspplt, out_layer="WYbhpts", outfolder=outfolder, 
...
## Display results
WYbhMWprj <- FIESTA:::crsCompare(WYbhMW, FIESTA:::rasterInfo(WYbhMW.fornf)$crs)$x
...
#   other_layers = NULL, puniqueid = "CN", savePOP = FALSE, evalid = NULL, 
...
#	  savexy = FALSE, outfolder = NULL, out_fmt = "csv", out_dsn = NULL,  
...
WYbhdat <- spGetPlots(bnd=WYbhfn, states="Wyoming", datsource="datamart", 
...
######################################################################################
# Usage:
# spGetAuxiliary(xyplt, xyplt_dsn = NULL, uniqueid = "PLT_CN", 
...
modeldat <- spGetAuxiliary(xyplt=WYspplt, uniqueid="CN", dunit_layer=WYbhfn, 
  dunitvar=NULL, rastlst.cont=rastlst.cont, rastlst.cont.name=rastlst.cont.name, 
...
unitmodel <- modeldat$dunitlut
unitvar <- modeldat$dunitvar
inputdf <- modeldat$inputdf
unitarea <- modeldat$dunitarea
...
######################################################################################
unitdat.bh <- spGetEstUnit(xyplt=WYplt, uniqueid="CN", 
    xvar="LON_PUBLIC", yvar="LAT_PUBLIC", xy.crs=4269, unit_layer=WYbhfn)
...
#   keepxy = FALSE, showext = FALSE, savedata = FALSE, exportsp = FALSE,  
#   exportNA = FALSE, outfolder = NULL, out_fmt = "shp", out_dsn = NULL, 
...
## Note: you can also use spGetAuxiliary() function for getting strata info
##    (only if strata layer is raster)
stratlst2 <- spGetAuxiliary(WYspplt, uniqueid="CN",
	dunit_layer=WYbhfn, dunitvar=NULL,
	rastlst.cat=fornffn, rastlst.cat.name="STRATUMCD",
	strata=TRUE, rast.strata=fornffn)
stratlst2$stratalut	
