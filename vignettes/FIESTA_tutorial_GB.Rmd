...
### Population Data {#popdat}
<details>
  <summary>View Population Data</summary>
...
* **poptables**   - A list of data tables used for estimates (cond, plt, pltassgn, tree, seed, vsubpspp, vsubpstr, subplot, subp_cond, lulc). See below for more details about tables.
...
* **popTableIDs**   - A list of data tables used for estimates (cuniqueid, puniqueid, pltassgnid, tuniqueid, vuniqueid, subpuniqueid="PLT_CN", lulcuniqueid="PLT_CN", condid="CONDID"). See below for more details about tables.

* **cuniqueid** - Unique identifier for plot in cond (default="PLT_CN").
* **puniqueid** - Unique identifier for plot in plt (default="CN").
* **pltassgnid**- Unique identifier for plot in pltassgn (default="PLT_CN").
* **tuniqueid** - Unique identifier for plot in tree or vsubpspp or vsubpstr.
* **vuniqueid** - Unique identifier for plot in tree or seed.
* **subpuniqueid** - Unique identifier for plot in subplot or subp_cond.
...
* **row.add0**      - TRUE, add 0 value in table for row values that occur in data set or in rowlut, but are not in final estimate.
* **col.add0**      - TRUE, add 0 value in table for column values that occur in data set or in rowlut, but are not in final estimate.
...
The following examples are for generating FIA's traditional, state-level estimates using standard FIA Evaluation data from FIA's National database, including FIA plot data and associated estimation unit and stratification information. The examples use data from three inventory years of field measurements in the state of Wyoming, from FIADB_1.7.2.00, last updated June 20, 2018, downloaded on June 25, 2018 and stored as internal data objects in `FIESTA`. Estimates using population data from FIADB can be compared with EVALIDator estimates, using the 2013 evaluation (https://apps.fs.usda.gov/Evalidator/evalidator.jsp).
...
WYbighorn_dem_250m.img    | Erdas Imagine raster of elevation change, in meters^4^
...
^4^USGS National Elevation Dataset (NED), resampled from 30m resolution to 250m. Projected in Albers Conical Equal Area, Datum NAD27 (U.S. Geological Survey 2017).
...
if (!dir.exists(paste0(getwd(),"/FIESTA"))) {
  dir.create(paste0(getwd(),"/FIESTA"))
}
if (!dir.exists(paste0(getwd(),"/FIESTA", "/outfolder"))) {
...
#### Example 1: Getting Population Data for Wyoming Area and Tree Estimates, Using Post-Stratification (FIADB match)

In this example, we use the sample WY data (2013 Evaluation) in `FIESTA` to generate population data for the GB module. We check this output with the FIADB pop_stratum table from FIA DataMart for 561301 Evalid, using the `FIESTA::DBqryCSV` function.
<details>
...
#### Example 2: Getting Population Data for Wyoming Area and Tree Estimates, Using Post-Stratification (Custom boundary)
...
#### Example 1a: Generate Estimate by Estimation Unit and Sum to Population (FIADB match)
<details>
  <summary>View Example</summary>

Using the `modGBarea` function we generate estimates by estimation unit (i.e., ESTN_UNIT) and sum to population (i.e., WY). `FIESTA` then returns raw data and titles for area of forest land, Wyoming, 2011-2013 (sum estimation units). Note that we set some options for our table output with the `table_opts` argument. For a full list of possible table options, you can run `help(table_options)`.
...
area1a <- modGBarea(
    ...
    table_opts = table_options(  # set up options for table output
...
To easily access our estimate and percent sampling error of estimate we can just grab the `est` object from out ouputted list: 
...
raw <- area1a$raw      # extract raw data list object from output
names(raw)
...
#### Example 1b: Adding Rows to the Output (FIADB match)
<details>
  <summary>View Example</summary>

In this example, we look at adding rows to the output: 
...
area1b  <- modGBarea(
    ...
    sumunits = TRUE,           # out - sum estimation units to population
    table_opts = table_options(  # set up options for table output
      rawdata = TRUE,            # out - return raw data
      returntitle = TRUE         # out - return title information
      )
...
#### Example 1c: Adding Rows and Columns to the Output (FIADB match)
<details>
  <summary>View Example</summary>
...
#### Example 2a: Generate Estimate for Bighorn National Forest
<details>
  <summary>View Example</summary>
...
area1 <- modGBarea(
    ...
    colvar="STDSZCD",            # est - column domain
    table_opts = table_options(  # set up options for table output
      row.FIAname=TRUE,          # out - return FIA row names
      col.FIAname=TRUE,          # out - return FIA column names
      rawdata = TRUE,            # out - return raw data
      returntitle = TRUE         # out - return title information
      )
    )
