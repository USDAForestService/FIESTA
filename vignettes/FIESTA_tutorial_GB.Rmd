...
The main objective of this tutorial is to demonstrate how to use `FIESTA` for generating estimates and estimated variances using FIA's 'Green Book' estimators with FIA's standard state-level population data (i.e., Evaluation) from the FIA database (FIADB), including FIA plot data, strata proportions, and area of estimation units. This tutorial steps through the following topics to help use `FIESTA`'s GB module. 
...
* **data_tables**   - A list of data tables used for estimates (cond, plt, pltassgn, tree, seed, vsubpspp, vsubpstr, subplot, subp_cond, lulc). See below for more details about tables.
...
* **data_uniqueids**   - A list of data tables used for estimates (cuniqueid, puniqueid, pltassgnid, tuniqueid, vuniqueid, subpuniqueid="PLT_CN", lulcuniqueid="PLT_CN", condid="CONDID"). See below for more details about tables.
...
#### Example 1: Getting Population Data for Wyoming Area and Tree Estimates, Using Post-Stratification
<details>
  <summary>View Example</summary>

First, we get population data for Wyoming area and tree estimates, using post-stratification with the `modGBpop` function:

```{r, results = FALSE, message = F}
GBpopdat <- modGBpop(
  data_tables = data_tables_list(cond = WYcond,             # FIA plot/condition data
                                 tree = WYtree,             # FIA tree data
                                 seed = WYseed,             # FIA seedling data
                                 pltassgn = WYpltassgn),    # plot assignments
  data_uniqueids = data_uniqueids_list(pltassgnid = "CN"),  # uniqueid of plots
  ...
  strata_opts = strata_options(stratalut = WYstratalut)     # strata classes and pixels counts
  )
```
...
#### Example 1: Generate Estimate by Estimation Unit and Sun to Population
<details>
  <summary>View Example</summary>

First, with the `modGBarea` function we generate estimates by estimation unit (i.e., ESTN_UNIT) and sum to population (i.e., WY). `FIESTA` then returns raw data and titles for area of forest land, Wyoming, 2011-2013 (sum estimation units). Note that we set some options for our table output with the `table_opts` argument. For a full list of possible table options, you can run `help(table_options)`.
...
#### Example 2: Adding Rows to the Output
<details>
  <summary>View Example</summary>
...
#### Example 3: Adding Rows and Columns to the Output
<details>
  <summary>View Example</summary>
