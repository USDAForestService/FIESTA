...
#######################################################################################
# Usage:
# modPBpop(pntdat = NULL, pltpct = NULL, plotid = "plot_id", pntid = NULL, 
#	  pltpctvars = NULL, plt = NULL, pltassgn = NULL, puniqueid = "CN", 
#	  pltassgnid = "CN", nonsamp.pfilter = NULL, strata = FALSE, sumunits = FALSE, 
#   unitvar = NULL, unitvar2 = NULL, unitarea = NULL, areavar = "ACRES",
#   areaunits = "acres", unitcombine = FALSE, minplotnum.unit = 10, 
#   stratalut = NULL, strvar = "STRATUMCD", getwt = TRUE, getwtvar = "P1POINTCNT", 
#   strwtvar = "strwt", stratcombine = TRUE, minplotnum.strat = 2, pvars2keep = NULL, 
#   saveobj = FALSE, objnm = "PBpopdat", savedata = FALSE, outfolder = NULL, 
#   out_fmt = "csv", out_dsn = NULL, outfn.pre = NULL, outfn.date = FALSE,
#   overwrite_dsn = FALSE, overwrite_layer = TRUE, PBstratdat = NULL, gui = FALSE)
# 
# Description:
...
PBpop <- modPBpop(pnt=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt")
names(PBpop)

PBx <- PBpop$PBx
pltassgnx <- PBpop$pltassgnx
stratalut <- PBpop$stratalut
...
#######################################################################################
# Usage:
# modPB(PBpopdat=NULL, tabtype = "PCT", sumunits = FALSE, strata = FALSE, 
#   strtype = "POST", ratio = FALSE, landarea = "ALL", landarea.filter = NULL, 
#   nonsamp.pntfilter = NULL, pntfilter = NULL, pfilter = NULL, rowvar = NULL,    
...
#   divideby = NULL, savedata = FALSE, outfolder = NULL, outfn.pre = NULL,
#   outfn.dat = FALSE, addtitle = TRUE, rawdata = FALSE, rawonly = FALSE, raw_fmt = "csv"
...
## Percent ##
# Percent land cover at Time 1 (2011) for all land in Davis and Salt Lake Counties, UT
cover1 <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)")
names(cover1)
cover1$est


# Note: pltassgn is not necessary for this estimate because strata is not included
cover1 <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt", 
    rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)")
names(cover1)
cover1$est


# Now, use the object from modGBpop for more efficient results and add rawdata
cover1 <- modPB(PBpopdat=PBpop,
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)",
		rawdata=TRUE)
names(cover1)
cover1$est
...
## Note: unit.totest should always equal 100% unless a filter is used to subset results
cover1.raw$unit.totest

## Detailed calculations for row estimates. See help(modPB) for variable descriptions 
cover1.raw$unit.rowest

## Name of rowvar
cover1.raw$rowvar

## The domain-level data used for estimate, with proportion of points by category
head(cover1.raw$pltdom.row)


# Note: An Uninterpretable class is included in the previous table. To remove, add nonsamp.pntfilter
cover1 <- modPB(PBpopdat=PBpop,
...
cover1.area <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", unitarea=sum(unitarea$ACRES))
cover1.area$est


# Note: adding area requires generating a new set of population data
PBpoparea <- modPBpop(pnt=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
        unitarea=sum(unitarea$ACRES))
names(PBpoparea)

head(PBpoparea$PBx)
head(PBpoparea$pltassgnx)
PBpoparea$stratalut
PBpoparea$unitarea
...
cover1.area <- modPB(PBpopdat=PBpoparea,
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.pntfilter="cover_1 != 999", tabtype="AREA")
cover1.area$est
...
## Percent ##
# Percent land cover at Time 1 (2011) for all land in Davis and Salt Lake Counties, UT
# Note: use the area population data for either percent or area estimates
rowvar1 <- "cover_1"

cover1 <- modPB(PBpopdat=PBpoparea,
		rowvar=rowvar1, rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.pntfilter="cover_1 != 999")
names(cover1)
cover1$est
...
## Percent ##
# Percent land cover at Time 1 (2011) for all land in Davis and Salt Lake Counties, UT
# Note: Save to outfolder, including rawdata (A new folder is created to store rawdata files)
cover1 <- modPB(PBpopdat=PBpoparea,
		rowvar=rowvar1, rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.pntfilter="cover_1 != 999", rawdata=TRUE, savedata=TRUE, outfolder=outfolder,
		returntitle=TRUE)		
names(cover1)
cover1$est
raw1 <- cover1$raw
titlelst1 <- cover1$titlelst
names(raw1)
names(titlelst1)

title.rowvar1 <- titlelst1$title.rowvar
unit.rowest1 <- raw1$unit.rowest
...
cover2 <- modPB(PBpopdat=PBpoparea,
		rowvar=rowvar2, rowlut=icecover_2, rawdata=TRUE, title.rowvar="Land Cover (2014)",
		returntitle=TRUE)
names(cover2)
cover2$est
raw2 <- cover2$raw
titlelst2 <- cover2$titlelst
names(raw2)
names(titlelst2)

title.rowvar2 <- titlelst2$title.rowvar
unit.rowest2 <- raw2$unit.rowest
...
## Display netchg in barplot (Time1/Time2)
tabvars <- c("est", "est.se")
tab1 <- unit.rowest1[, c(rowvar1, title.rowvar1, tabvars)]
setnames(tab1, tabvars, paste0(tabvars, ".1"))

tab2 <- unit.rowest2[, c(rowvar2, title.rowvar2, tabvars)]
setnames(tab2, tabvars, paste0(tabvars, ".2"))

tabx <- merge(tab1, tab2, by.x=rowvar1, by.y=rowvar2)
sevar <- names(tabx)[grepl("est.se", names(tabx))]
yvar <- names(tabx)[grepl("est.", names(tabx)) & !names(tabx) %in% sevar]

datBarplot(tabx, yvar=yvar, xvar=title.rowvar1, errbars=TRUE,
		sevar=sevar, ylabel="Percent", addlegend=TRUE, 
 		args.legend=list(x="topleft", bty="n", cex=.8, legend=c("Time1", "Time2")),
		main=substr(titlelst1$title.row, 1, nchar(titlelst1$title.row)-7))
...
chg_ag.pnts <- modPB(PBpopdat=PBpoparea,
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
		pntfilter=pntfilter, returntitle=TRUE, title.filter="observed changed")
names(chg_ag.pnts)
chg_ag.pnts$est
...
## Create barplot of area estimates
unit.rowest <- chg_ag.pnts$raw$unit.rowest
datBarplot(unit.rowest, xvar="Agent of Change", yvar="est", 
  errbars=TRUE, sevar="est.se", ylab="Percent",
...
# Note: Add a landarea filter to subset dataset to only plots with observed change.
landarea.filter <- "change_1_2 == 1"
chg_ag.plts.area <- modPB(PBpopdat=PBpoparea,
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
...
# Note: Add a point filter to include observed change on points
pntfilter <- "chg_ag_2 > 0"
chg_ag.pnts.area <- modPB(PBpopdat=PBpoparea,
		rowvar="chg_ag_2", rowlut=icechg_ag, rawdata=TRUE, title.rowvar="Agent of Change",
		pntfilter=pntfilter, returntitle=TRUE, title.filter="observed changed", 
...
chg_ag_cover1 <- modPB(PBpopdat=PBpoparea, 
    rowvar="chg_ag_2", rowlut=icechg_ag, colvar="cover_1", collut=icecover_1, 
    rawdata=TRUE, title.rowvar="Change agent", title.colvar="Land cover (2011)", 
    returntitle=TRUE)
chg_ag_cover1$est
...
chg_ag_cover1.acres <- modPB(PBpopdat=PBpoparea, 
    rowvar="chg_ag_2", rowlut=icechg_ag, colvar="cover_2", collut=icecover_2, 
    rawdata=TRUE, title.rowvar="Change agent", title.colvar="Land cover (2011)", 
    returntitle=TRUE, tabtype="AREA", unitarea=sum(unitarea$ACRES))
...
chg_ag_cover2 <- modPB(PBpopdat=PBpoparea, 
    rowvar="chg_ag_2", rowlut=icechg_ag, colvar="cover_2", collut=icecover_2, 
    rawdata=TRUE, title.rowvar="Change agent", title.colvar="Land cover (2014)", 
    returntitle=TRUE)
chg_ag_cover2$est
chg_ag_cover2$pse
...
cover1.unit.area <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id",  
    pntid="dot_cnt", rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
    nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", unitarea=unitarea, unitvar="ESTN_UNIT", 
    areavar="ACRES")
cover1.unit.area$est
cover1.unit.area$pse
   
   
# Note: Need to calculate new population data for generating estimates by estimation unit
PBpopunit <- modPBpop(pnt=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
        unitarea=unitarea, unitvar="ESTN_UNIT")
names(PBpopunit)

head(PBpopunit$PBx)
head(PBpopunit$pltassgnx)
PBpopunit$stratalut
PBpopunit$unitarea

## Check PBpopunit
cover1.unit.areab <- modPB(PBpopdat=PBpopunit,
    rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
    nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", 
    unitarea=unitarea, unitvar="ESTN_UNIT", areavar="ACRES")
cover1.unit.areab$est
cover1.unit.areab$pse
...
cover1.unitsum <- modPB(PBpopdat=PBpopunit,
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", unitarea=unitarea, 
...
# Return raw data
# Note: Estimates are generated by ESTN_UNIT and summed to the population.
cover1.unitsum.area <- modPB(PBpopdat=PBpopunit,
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)", 
		nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", unitarea=unitarea, 
		unitvar="ESTN_UNIT", areavar="ACRES", sumunits=TRUE, rawdata=TRUE)
...
cover12 <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		...
		nonsamp.pntfilter="cover_1 != 999", returntitle=TRUE)
names(cover12)
cover12$est
cover12$pse

## Check using PBpoparea object from modPB()
...
## Summed proportions for row totals
head(cover12.raw$pltdom.row)

## Summed proportions for column totals
head(cover12.raw$pltdom.col)

## Summed proportions for transition (row and column)
head(cover12.raw$pltdom.grp)

## Estimates for row totals (Time 1)
cover12.raw$unit.rowest

## Estimates for column totals (Time 2)
cover12.raw$unit.colest
...
cover12.acres <- modPB(PBpopdat=PBpoparea,
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, title.rowvar="Land Cover (2011)", title.colvar="Land Cover (2012)", 
		nonsamp.pntfilter="cover_1 != 999", tabtype="AREA", unitarea=sum(unitarea$ACRES))
names(cover12.acres)
...
## Check to make sure pse is the same for percent and area
cover12$pse
cover12.acres$pse
...
cover12nm <- modPB(pntdat=icepnt2, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
...
cover12.lt200 <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, title.rowvar="Land Cover (2011)", title.colvar="Land Cover (2012)", 
		nonsamp.pntfilter="cover_1 != 999", pntfilter="cover_1 < 200", 
...
cover12b <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, title.rowvar="Land Cover (2011)", title.colvar="Land Cover (2014)", 
		nonsamp.pntfilter="cover_1 != 999", gainloss=TRUE)
names(cover12b)
...
chgcover1 <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		ratio=TRUE, rowvar="change_1_2", rowlut=changelut, colvar="cover_1", 
		collut=icecover_1, nonsamp.pntfilter="cover_1 != 999", rawdata=TRUE, 
		title.rowvar="Change")
chgcover1$est
chgcover1$pse
...
chgcover1.grp <- modPB(pntdat=icepnt2, puniqueid="plot_id", pntid="dot_cnt",
...
chg_ag_cover1.rat <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		ratio=TRUE, rowvar="chg_ag_2", rowlut=icechg_ag, colvar="cover_1", collut=icecover_1, 
		rawdata=TRUE, nonsamp.pntfilter="cover_1 != 999", title.rowvar="Change agent", 
...
chg_ag_cover1.rat.acres <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		ratio=TRUE, rowvar="chg_ag_2", rowlut=icechg_ag, colvar="cover_1", collut=icecover_1, 
		rawdata=TRUE, nonsamp.pntfilter="cover_1 != 999", title.rowvar="Change agent", 
...
cover1_2.rat <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		ratio=TRUE, rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, nonsamp.pntfilter="cover_1 != 999", title.rowvar="Land cover (2011)", 
...
cover1_2.acres.rat <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		ratio=TRUE, rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, nonsamp.pntfilter="cover_1 != 999", title.rowvar="Land cover (2011)", 
...
cover12ps <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
...
## Create barplot with estimates and error bars (use Percent Sampling Error column)
datBarplot(x=x, xvar="Land cover (2011)", yvar="Estimate", errbars=TRUE, psevar="Percent Sampling Error")

## You can use data from the raw tables as well
x2 <- pltpct1$raw$unit.rowest

## Create barplot with estimates and error bars (using se column) and add labels and titles
datBarplot(x=x2, xvar="Land cover (2011)", yvar="est", errbars=TRUE, sevar="est.se",
  ylim=c(0,30), ylabel="Percent of land", main="Percent cover at Time 1 (2011)")


## Barplot of land cover, Time 1 and Time 2
##############################################################################################
## Let's look at estimates at Time 1 compared to estimates at Time 2
# Percent land cover at Time 1 (2011) for all land in Davis and Salt Lake Counties, UT
cover1 <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, title.rowvar="Land Cover (2011)")
names(cover1)
...
# Percent land cover at Time 2 (2014) for all land in Davis and Salt Lake Counties, UT
cover2 <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		rowvar="cover_2", rowlut=icecover_2, rawdata=TRUE, title.rowvar="Land Cover (2014)")
names(cover2)
cover2 <- cover2$est

## First, let's change the column names and merge the tables to 1 table (removing Uninterpretable class)
setnames(cover1, c("Land Cover", "est11", "pse11"))
setnames(cover2, c("Land Cover", "est14", "pse14"))
cover <- merge(cover1, cover2, by="Land Cover")

## Next, we will remove the Total row and reorder
cover <- cover[c(6,4,3,1,2,7),]

## Create barplot with estimates and error bars (using pse columns)
datBarplot(x=cover, xvar="Land Cover", yvar=c("est11", "est14"), errbars=TRUE, psevar=c("pse11", "pse14"),
  ylim=c(0,30), ylabel="Percent of land", main="Percent cover at Time 1 (2011) and Time 2 (2014)")
  

## Let's add a legend to the bar plot, specifying the location, the text, and no surrounding box
datBarplot(x=cover, xvar="Land Cover", yvar=c("est11", "est14"), errbars=TRUE, psevar=c("pse11", "pse14"),
  ylim=c(0,30), ylabel="Percent of land", main="Percent cover at Time 1 (2011) and Time 2 (2014)", addlegend=TRUE,
  args.legend=list(x="topleft", legend=c("2011", "2014"), bty="n"))
  

##################
###################
## Ratio ## - Transitional change within category
# Percent land cover at Time 2 (2014) within land cover at Time 1 in Davis and Salt Lake Counties, UT 
cover1_2.rat <- modPB(pntdat=icepnt, puniqueid="plot_id", pntid="dot_cnt",
		ratio=TRUE, rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
		rawdata=TRUE, nonsamp.pntfilter="cover_1 != 999", title.rowvar="Land cover (2011)", 
		title.colvar="Land cover (2014)", returntitle=TRUE)
cover1_2.rat$raw$unit.grpest
cover1_2.rat$titlelst

x <- cover1_2.rat$raw$unit.grpest

datBarStacked(x=x, main.attribute="Land cover (2011)", sub.attribute="Land cover (2014)",
      response="est", xlabel="Land Cover (2011)", legend.title="Land Cover (2014)")
      
 
## Now, let's only look at change
x2 <- x[x$'Land cover (2011)' != x$'Land cover (2014)',]
datBarStacked(x=x2, main.attribute="Land cover (2011)", sub.attribute="Land cover (2014)",
      response="est", xlabel="Land Cover (2011)", legend.title="Land Cover (2014)",
...
## Gain / Loss ## - The difference in proportion between Time 1 and Time 2
# Percent land cover at Time 1 (2011) by land cover at Time 2 (2014) for all land in Davis and Salt Lake Counties, UT
# Return gain/loss estimates for each category in rawdata tables
cover12b <- modPB(pntdat=icepnt, pltassgn=iceplt, pltassgnid="plot_id", pntid="dot_cnt",
		rowvar="cover_1", rowlut=icecover_1, colvar="cover_2", collut=icecover_2, 
